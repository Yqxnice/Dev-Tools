...
watch(() => app.currentFeature, async (feat) => {
  if (feat === 'password-reset' && !mysql.versionInfo) await mysql.detectMySQL()
})
...
async function handleReset() {
  ...
  try {
    ...
    await mysql.resetMySQLPassword(fd.newPassword, mysql.selectedPasswordInstance, overridePort)
...
<template>
  <div class="feature-panel">
    <n-alert v-if="app.isGuestMode" type="info" :bordered="false" class="mb-3"><template #header>游客模式</template>无法执行密码重置</n-alert>
    ...
    <div style="margin-bottom:16px">
      <h3 style="font-size:18px;font-weight:600;margin:0 0 4px 0">MySQL 密码重置</h3>
      <p style="font-size:13px;color:var(--text-secondary);margin:0">重置 MySQL root 用户密码（无需原密码）</p>
    </div>
    ...
    <div style="margin-bottom:16px">
      <div v-if="mysql.selectedPasswordInstance && !mysql.selectedPasswordInstance.port" class="form-group">
        <label>手动指定端口</label>
        <n-input v-model:value="mysql.formData.passwordReset.manualPort" type="number" placeholder="默认 3306" />
      </div>
      <div class="form-group">
        <label>新密码</label>
        <n-input v-model:value="mysql.formData.passwordReset.newPassword" type="password" show-password-on="click" placeholder="至少 8 个字符" />
      </div>
      <div class="form-group">
        <label>确认密码</label>
    ...
    <div style="margin-top:12px">
      <n-button type="primary" :disabled="!app.isAdmin || app.isGuestMode" :loading="app.globalLoading" @click="handleReset">
        {{ app.globalLoading ? '重置中...' : '重置密码' }}
      </n-button>
    </div>

    <n-alert v-if="mysql.tempPassword" type="info" :bordered="false" style="margin-top:12px">
      <template #header>提示</template>临时密码已记录，切换至"密码修改"时将自动填充
    </n-alert>
  </div>
</template>
