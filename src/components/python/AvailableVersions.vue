...
async function handleRefresh() {
  try {
    await py.loadAvailableVersions()
...
async function doDownload(version: string): Promise<void> {
  ...
  try {
    ...
    task.setDownloadedPath(`${py.downloadPrefix}${version}`, path)
...
async function downloadLocal(version: string) {
  const taskId = `${py.downloadPrefix}${version}`
...
<template>
  <FlatTablePanel
    title="可用 Python 版本"
    description="查看所有可用的 Python 版本并下载安装包（下载可在后台进行，进度请前往下载中心查看）"
    :items="py.availableVersions as AvailablePythonVersion[]"
    :loading="py.loading"
  ...
  >
    <template #row="{ item }">
      <div class="version-row-info">
        ...
        <n-tag
          :type="item.is_stable ? 'success' : 'default'"
          size="small"
        >
          {{ item.is_stable ? '稳定版' : '预览版' }}
        </n-tag>
      </div>
      <n-space>
        <n-button
